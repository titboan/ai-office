...
- `/products` — каталог товаров (display_name, артикулы WB/Ozon, с/с)
- `/add` — мастер добавления товара в реестр (пошаговый диалог с кнопками)
- `/cost` — задать себестоимость (кнопки выбора товара → ввод суммы); быстрый путь: `/cost <ident> <сумма>`
- `/map name=X wb=Y ozon=Z` — добавить/обновить товар текстовой командой
...
product_mapping          -- реестр товаров: wb_article, ozon_offer_id, ozon_sku, display_name
                            wb_article и ozon_offer_id — nullable (товар может быть на одной ПЛ)
                            UNIQUE(display_name) — ключ для /add и /cost
product_costs            -- себестоимость: mapping_id → cost (₽), updated_at
...
- CSV — отчёт по SKU: campaign_id парсится из первой строки («№ XXXXX»), метрики агрегируются по строкам SKU; строка «Всего» пропускается
- GET /api/client/campaign со state=CAMPAIGN_STATE_RUNNING возвращает ~88 кампаний; реальных ~19 — остальные REF_VK (advObjectType=REF_VK, PaymentType=CAMPAIGN_TYPE_INVALID) → фильтруются в get_ad_stats
- Ручной триггер рекламного синка: `/sync_adv` (cmd_sync_adv → sync_ad_stats)
- Фикс голосовых в группе: base_agent.handle_voice не вызывает handle_message в группах — это делает Max._handle_group_message с проверкой триггера (has_mention / starts_with_max / is_reply_to_bot)
...
## Агент Питер — Бизнес-аналитик

### Что умеет
- Анализирует продажи WB+Ozon из БД за период
- Считает валовую рентабельность (выручка − себестоимость, без комиссий МП — неизвестны)
- Считает ДРР по площадкам из marketplace_adv_stats
- Показывает топ товаров по display_name из product_mapping
- Даёт план действий с конкретными цифрами

### Команды Питера
- `/report [цель=200000] [период=14]` — полный анализ магазинов с планом до цели
- `/analyze <вопрос>` — произвольный бизнес-анализ

### Данные для анализа
- `marketplace_orders` — заказы WB+Ozon
- `marketplace_adv_stats` — рекламные расходы WB+Ozon
- `marketplace_stocks` — остатки
- `product_mapping` — display_name товаров
- `product_costs` — себестоимость

### Известные ограничения
- Рентабельность = валовая (без комиссий МП и логистики МП — парсинг финотчётов не реализован)
- Ozon-маржа считается только если заполнен ozon_sku в product_mapping
- ДРР считается на уровне площадки, не товара (кампании не привязаны к SKU)
- max_tokens=4096 для /report (глобальный дефолт 2048 переопределён)
...
- [x] **Ozon реклама** — ZIP→UTF-8 BOM парсер, фильтр REF_VK, работает с 09.06
- [x] **Каталог товаров у Макса** — product_mapping (реестр), product_costs, мастера /add и /cost
- [x] **Питер-аналитик** — /report с рентабельностью, ДРР, планом до цели

### 🟡 Следующие шаги

- [ ] Парсинг финотчётов WB+Ozon → реальные комиссии и логистика в расчёт рентабельности
- [ ] /sync_sku у Макса — автозаполнение ozon_sku в product_mapping из Ozon API
- [ ] Execution Trace (вариант А) — SQL-вьюха дерева задач по chain_id для отладки
