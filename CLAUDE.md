...
wb_campaigns             -- названия кампаний WB (вручную, т.к. API 404)
product_mapping          -- WB артикулы ↔ Ozon offer_id (для DataLens)
stocks_unified           -- VIEW: объединяет WB и Ozon остатки
adv_stats_unified        -- VIEW: объединяет рекламу WB и Ozon
adv_stats_summary        -- VIEW: агрегат по кампаниям
```
...
- Остатки: POST `/v2/analytics/stock_on_warehouses` + маппинг через `/v3/product/info/list`
...
- Токен: POST → кешируется в Redis 25 мин через `_get_token()`
- Статистика: POST → UUID → GET poll → CSV (Windows-1251 encoding)

### DataLens дашборд

- **URL:** https://datalens.yandex/zhnao5ut1xvmj
- **Подключение:** maglev.proxy.rlwy.net:12614, база railway
- Чарты: заказы, выручка, остатки, отзывы (WB), реклама (CTR, клики, показы)
...
- Реклама WB: `marketplace_adv_stats`, DataLens дашборды
- Реклама Ozon: Performance API + OAuth, Redis кеш токена
- DataLens: заказы, выручка, остатки, отзывы, реклама
- Ева — код готов, ожидает TELETHON_SESSION

### 🔵 В работе

- [ ] **Ева** — второй номер телефона → TELETHON_SESSION
- [ ] **Ozon остатки** — исправить дублирование (числовые SKU vs offer_id)
- [ ] **DataLens** — убрать Null в остатках Ozon после fix БД
