...
| 📰 Ева | Сводка Telegram каналов | eva | Telethon MTProto, Notion Content (ждёт TELETHON_SESSION) |
...
Фоновые задачи:
  ...
  ├── Ева — дайджест каналов: 09:30 МСК (ждёт TELETHON_SESSION)
...
- [ ] **Execution Trace** — таблица `task_events` в Postgres.
...
- [ ] **Cost Tracking** — `estimated_cost` и `latency_ms` в БД.
  Сейчас только логирование токенов — стоимость не считается.
  Отчёт от Питера по запросу.

- [ ] **Деплой Евы** — добавить `TELETHON_SESSION` в Railway, проверить дайджест.

- [ ] **Деплой Тины** — добавить `TINA_BOT_TOKEN` в Railway, проверить `/tenders`.

- [ ] CI/CD для Кевина (триггер после мёрджа PR) — отложено
