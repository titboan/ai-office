...
> Обновление 2026-07-13: расследование нашло и починило действующие баги маршрутизации (фоновые задачи иногда отвечали через чужой бот вместо Марты), добавлены недостающие прокси-команды (`/set`, `/roadmap`, `/tender`) и зеркала callback-кнопок Макса/Питера на Марте, добавлен форвардинг текстового продолжения мастеров (`/add`, `/cost`, `/reprice`) через Марту. Регистрация самих `ConversationHandler`-ов и отключение polling отдельных ботов — не сделаны, это следующий шаг. Подробности и статус по фазам — `plans/2026-06-24-single-entry-marta.md`.
...
Сделано: маршрутизация ответов фоновых/scheduled задач через `MARTA_BOT_TOKEN`, полный паритет команд, зеркала callback-кнопок Макса и Питера, форвардинг текста для мастеров Макса (интерим-решение — сами `ConversationHandler` пока живут в App Макса).

Осталось:
- [ ] Перенести регистрацию `ConversationHandler`-ов Макса (`/add_shop`, `/cost`, `/map`) в Мартину App — сейчас только текст-продолжение форвардится, сама регистрация ещё на боте Макса
- [ ] Убрать `start_polling` у отдельных ботов (оставить только worker loop) — высокий риск, отдельная фаза
- [ ] Убрать отдельные токены (оставить только `MARTA_BOT_TOKEN`) — после подтверждения, что никто не пишет ботам напрямую

Триггер для завершения: первый внешний клиент (SaaS).
