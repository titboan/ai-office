...
Статус: в работе (осталась только живая проверка /tenders в Telegram, см. Фазу 6; дублирует
Фазу 5 в plans/2026-06-12-tina-zapusk.md)
...
- [x] Деплой — подтверждено 2026-07-02 по факту (см. ROADMAP.md: «🏛️ Тина ... ✅ Активна
  (TINA_BOT_TOKEN добавлен)»; `TinaAgent` зарегистрирован в `main.py` AGENTS, дайджест-луп
  подключён, `tender_opportunities` есть в схеме `db.py`)
- [ ] Живая проверка `/tenders` через Telegram — не делал в этой сессии, нет доступа к Telegram
