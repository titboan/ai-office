...
- [x] **Фаза 7**: Настройки поиска через форму в Mini App — см. `plans/2026-06-30-tina-tender-settings-mini-app.md`
  - Ключевые слова, бюджет НМЦК и регион поиска теперь редактируются пользователем через форму в Telegram Mini App, без правки `config.py`
