...
- [ ] **Фаза 8**: Image-to-image — реальное фото товара вместо выдуманного AI

  Сейчас Дэн рисует инфографику «с нуля» по тексту (`generate_image`, только промпт) — реальный товар не участвует, на слайдах может оказаться AI-похожий, но не настоящий предмет. Нужно уметь опираться на уже загруженное фото товара с карточки WB/Ozon (image-to-image редактирование), чтобы на слайдах был именно тот товар, который продаётся.

  - [x] **Фаза 8а**: Инструменты чтения фото и редактирования изображения (без подключения к Дэну)
    - `WBClient.get_card_photo_url(nm_id) -> str | None` — текущее главное фото карточки (`content-api.wildberries.ru/content/v2/get/cards/list`, `photos[0].big`)
