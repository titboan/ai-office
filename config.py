...
class Config:
    ...
    # Генерация изображений (Дэн) — RU-реселлер OpenAI-совместимого API, AITUNNEL
    # (дешевле и понятнее по цене за изображение, чем ProxyAPI — см. plans/2026-07-19-dan-marketplace-funnel.md)
    IMAGE_GEN_API_KEY: str = os.getenv("IMAGE_GEN_API_KEY", "")
    IMAGE_GEN_BASE_URL: str = os.getenv("IMAGE_GEN_BASE_URL", "https://api.aitunnel.ru")
...
# Sanity-check ручного ввода цены товара — защита от опечатки (лишний ноль/цифра),
# не бизнес-правило
config.PRICE_SANITY_MAX_MULTIPLIER = 3   # новая цена не должна быть более чем в N раз выше текущей

# Допуск для сигнала "ставка выше/ниже рынка" (рекомендованная ставка WB) —
# насколько предложенный CPM может отклониться от рыночной рекомендации WB,
# прежде чем предупреждать о риске перерасхода/проигрыша аукциона
...
Позиционирование: премиум-сегмент — не самая низкая цена на рынке, аргумент для клиента
не в скидке, а в качестве и составе. При анализе цен и рекламы не демпинговать в сторону
экономичных конкурентов — конкурируем качеством, а не ценой.

Почему клиент выбирает DoggyDog, а не конкурента:
- Натуральность состава — без химии и лишних добавок, честный состав
- Уникальный ассортимент — позиции вроде бычьего корня и говяжьей трахеи редко встречаются
  у других продавцов в нише
- Корм холистик — отдельное премиальное направление, не только лакомства

Товары:
- Лакомства для собак (сушёные): говяжье лёгкое, бычий корень, трахея говяжья
- Корм холистик для собак: говядина, индейка

Тон текстов (ответы на отзывы, карточки товара, коммуникация с клиентом): дружелюбный,
с акцентом на заботу и любовь к питомцу — не сухой и не казённый.
...
# Оркестрация цепочек агентов (chain planner Марты, _advance_chain)
# 10 минут — с запасом покрывает самые долгие обычные шаги (Дэн: генерация картинки до
# ~600с timeout) плюс время на реакцию воркер-лупа (тик каждую итерацию), но не настолько
# долго, чтобы пользователь долго гадал, работает ли цепочка ещё
config.CHAIN_STALL_TIMEOUT_MINUTES = 10  # если цепочка не продвинулась N минут — считаем зависшей, уведомляем пользователя
