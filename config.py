...
# Окно расчёта темпа продаж (шт/день) по умолчанию — единое для /supply, сток-алертов
# и сигнала "дней остатков" в /reprice. /order у Питера намеренно использует более длинное
# окно (30 дней по умолчанию, настраивается период=) — там нужен сглаженный месячный тренд,
# а не быстрая реакция как в алертах.
config.SALES_VELOCITY_WINDOW_DAYS = 14
