...
## Онбординг нового магазина (/start)

При первом подключении магазина (после ввода токенов, `_send_finish` →
`_run_post_onboarding_flow`, фоновой задачей через `asyncio.create_task` —
НЕ await в обработчике апдейта, иначе бот встанет для всех чатов, PTB тут
без `concurrent_updates=True`) пользователю НЕ нужно вручную вызывать
команды — Макс сам, по шагам, с сообщениями о прогрессе:
1. `sync_marketplace_data` — заказы/остатки
2. `db.auto_populate_product_mapping` — заводит товары в `product_mapping`
   из синка (без ручного `/add`)
3. `sync_financial_report(days=90)`, `sync_ad_stats`, `sync_funnel`
4. проактивный мастер себестоимости (`_run_cost_wizard`) — бот сам
   спрашивает по одному товару без заданной с/с, пока не пройдёт весь
   список (`/skip`/`/done` — управление)

См. `plans/2026-07-14-guided-onboarding-analytics.md`. Команды ниже —
для ручного повторного запуска/донастройки после первого онбординга.

## Один товар — разные артикулы на WB и Ozon

Если у продавца товар физически один, но артикулы разные (например
`wb_article="БК50гр"`, `ozon_offer_id="КБ50"`) — `auto_populate_product_mapping`
намеренно заводит его ДВУМЯ строками в каталоге (мэтчинг только по точному
совпадению `display_name`, без фаззи — риск склеить себестоимость чужих
товаров). Чтобы аналитика/ABC/маржа считались по одной реальной позиции, а
не по двум "половинкам":

1. **Автоматически, по штрихкоду.** После каждого `/sync` (и в фоновом синке
   при онбординге) Макс сам ищет пары товаров с пересекающимися штрихкодами
   (`db.find_barcode_merge_candidates`, штрихкоды собираются
   `db.collect_and_save_barcodes` без новых API-вызовов — уже есть в ответах
   `/api/v1/supplier/stocks` у WB и `/v3/product/info/list` у Ozon) и
   спрашивает подтверждение кнопками Да/Нет — никогда не сливает втихую
   (ошибка слияния бьёт по себестоимости/марже). Callback `merge:yes:...`/
   `merge:no:...` → `_handle_merge_callback`.
2. **Вручную, если штрихкода нет.** `/merge_products` — визард: выбрать
   товар WB → выбрать товар Ozon → подтвердить → `db.merge_product_rows`.
   Callback-префикс `mergewiz:` (не путать с `merge:` из автопредложения —
   разные механизмы, оба ведут к одной и той же `merge_product_rows`).

См. `plans/2026-07-14-cross-marketplace-product-merge.md`. Известное
ограничение: неотвеченное автопредложение повторится на следующем `/sync`
(нет трекинга "уже показано, ждём ответа") — не критично, кнопки в старых
сообщениях остаются рабочими.
...
- `/merge_products` — вручную объединить WB- и Ozon-версии одного товара
  с разными артикулами (fallback, когда штрихкод не совпал/не заполнен)
- `/cancel` — отменить активный мастер (в т.ч. проактивный мастер себестоимости
  и мастер объединения товаров)
