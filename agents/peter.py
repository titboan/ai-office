...
Для каждого товара КРИТИЧНО/СРОЧНО — кластеры построчно (из stock + аналитики складов):
...
Для mkt_supplies Ozon показывай склад назначения (warehouse_name) если он есть:
  ...
  🚛 В пути → [склад]: [N] шт
Если clusters содержит только "Прочие" — выводи как есть, не используй WB как прокси.
**→ заказать [N] шт (распред.: [Кластер1] [N] шт, [Кластер2] [N] шт)**
