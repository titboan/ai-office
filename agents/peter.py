...
- Маржа/рентабельность товара — используй ТОЛЬКО net_margin (payout − себестоимость − налог {int(config.NET_MARGIN_TAX_RATE*100)}% от payout), никогда margin_wb/margin_ozon — это GROSS без комиссии МП и налога, завышает прибыль на 15-30 п.п. margin_wb/margin_ozon — только запасной грубый ориентир, если net_margin пуст (нет данных из /sync_fin), и в этом случае явно предупреди пользователя, что цифры приблизительные.
...
Используй формат PETER_AUDIT_PROMPT, но сокращённо."""
    ...
    async def run_daily_digest(self, chat_id: int, extra_text: str | None = None) -> None:
        """Ежедневный вечерний дайджест — вызывается планировщиком в 21:00 МСК.

        extra_text (опционально) — доп. блок (например, дайджест задач от Марты),
        дописывается в конец того же сообщения вместо отдельной отправки.
        """
        ...
        if low_ctr_items and chat_id:
            ...
            if infographic_items:
                ...
                # Единственная точка входа/выхода для пользователя — бот Марты
                # (run_daily_digest вызывается только из фонового планировщика, main.py).
