...
PETER_NICHE_PROMPT = """Ты оцениваешь перспективность ниши товара перед закупкой — по реальным
данным публичного поиска Wildberries (не выдумывай цифры, только то, что дано).
...
**Предложений на WB:** N | **Цены:** от X ₽ до Y ₽ (медиана Z ₽)
**Средний рейтинг:** R | **Суммарно отзывов:** N (грубый индикатор активности спроса)

---

## Концентрация рынка

[перечисли топ-бренды и их долю от всех предложений]

> Если топ-5 брендов > 60% предложений — рынок концентрирован, зайти новичку сложнее.
> Если < 30% — рынок раздроблен, ниша более доступна для входа.

---

## 🎯 Вывод

[1-2 предложения: перспективно / рискованно / нужно больше данных — и почему,
опираясь ТОЛЬКО на цифры выше]

ПРАВИЛА:
- Никогда не оценивай маржу и не предлагай цену продажи — для этого нет данных
  (неизвестна ни реальная комиссия площадки по категории, ни себестоимость непроданного товара)
- Если offers_count = 0 — так и скажи: "по этому запросу WB не нашёл предложений", без домыслов
- total_reviews — это сумма отзывов по всем найденным карточкам, не за период; не выдавай за динамику продаж"""
...
class PeterAgent(BaseAgent):
    ...
    async def handle_task(self, task: str, from_agent: str = "user") -> str:
        ...
        if not prompt:
            supply_ctx = ""
            if chat_id:
                _lead = await self._get_lead_days(chat_id)
                _safety = await self._get_safety_days(chat_id)
                supply_ctx = (
                    f"\n\nНАСТРОЙКИ ПОСТАВОК ПОЛЬЗОВАТЕЛЯ:\n"
                    f"lead_time = {_lead} дн (от заказа поставщику до склада МП)\n"
                    f"safety = {_safety} дн (страховой буфер)\n"
                    f"threshold = {_lead + _safety} дн (порог алерта по остаткам)"
                )
            prompt = f"Аналитическая задача от {from_agent}: {task}{data_str}{supply_ctx}"
        try:
...
Используй формат PETER_DRR_PROMPT."""
    ...
    async def cmd_find_niche(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ) -> None:
        """/find_niche <ключевое слово> — оценка ниши по публичным данным WB перед закупкой."""
        keyword = " ".join(context.args) if context.args else ""
        if not keyword:
            await update.message.reply_text(
                "<b>Использование:</b> /find_niche &lt;ключевое слово&gt;\n"
                "Пример: /find_niche лежанка для собаки",
                parse_mode="HTML",
            )
            return

        await update.message.reply_text(f"🔎 Смотрю на WB, что уже продаётся по «{keyword}»…")

        from tools.marketplace import WBClient
        from tools.niche_analysis import summarize_niche_competition

        try:
            rows = await WBClient("").get_competitor_prices(keyword, limit=100)
        except Exception as e:
            logger.error(f"[Питер/find_niche] ошибка WB поиска: {e}", exc_info=True)
            await update.message.reply_text(f"❌ Ошибка запроса к WB: {e}")
            return

        summary = summarize_niche_competition(rows)

        prompt = f"""Оцени нишу по запросу «{keyword}».

ДАННЫЕ (реальные, из публичного поиска WB):
{json.dumps(summary, ensure_ascii=False, default=str, indent=2)}

Используй формат PETER_NICHE_PROMPT."""
        ...
        try:
            ...
            resp = await client.messages.create(
                model=config.CLAUDE_MODEL,
                max_tokens=1024,
                system=PETER_NICHE_PROMPT,
                messages=[{"role": "user", "content": prompt}],
            )
            answer = resp.content[0].text
        except Exception as e:
            logger.error(f"[Питер/find_niche] ошибка Claude: {e}", exc_info=True)
            await update.message.reply_text(f"❌ Ошибка анализа: {e}")
            return
...
Только факты и цифры. Если нет проблем — «Всё в норме».{elina_note}"""
    ...
    async def _handle_peter_menu_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        if cmd == "settings":
            ...
            preset_markup = InlineKeyboardMarkup([
                [
                    InlineKeyboardButton("📦 7 дн",  callback_data="pset:lead:7"),
                    InlineKeyboardButton("📦 14 дн", callback_data="pset:lead:14"),
                    InlineKeyboardButton("📦 21 дн", callback_data="pset:lead:21"),
                    InlineKeyboardButton("📦 30 дн", callback_data="pset:lead:30"),
                ],
                [
                    InlineKeyboardButton("🛡 7 дн",  callback_data="pset:safety:7"),
                    InlineKeyboardButton("🛡 14 дн", callback_data="pset:safety:14"),
                    InlineKeyboardButton("🛡 21 дн", callback_data="pset:safety:21"),
                    InlineKeyboardButton("🛡 30 дн", callback_data="pset:safety:30"),
                ],
            ])
            await query.message.reply_text(text, parse_mode="HTML", reply_markup=preset_markup)
            return
    ...
    async def _handle_peter_set_callback(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        """Обработчик пресет-кнопок настроек поставок (pset:lead|safety:N)."""
        query = update.callback_query
        await query.answer()
        ...
        from db import set_user_setting
        if key == "lead":
            await set_user_setting(chat_id, "supply_lead_days", str(val))
            label = f"📦 Срок доставки: <b>{val} дн</b>"
        else:
            await set_user_setting(chat_id, "supply_safety_days", str(val))
            label = f"🛡 Страховой запас: <b>{val} дн</b>"
        lead   = await self._get_lead_days(chat_id)
        safety = await self._get_safety_days(chat_id)
        await query.message.reply_text(
            f"✅ {label}\n"
            f"<i>Порог алерта теперь: {lead + safety} дн</i>\n"
            f"<i>Применяется в /supply, /order и сток-алертах.</i>",
            parse_mode="HTML",
        )
    ...
    def _bot_commands(self) -> list:
        ...
        return [
            ...
            BotCommand("find_niche", "Оценить нишу товара перед закупкой (по данным WB)"),
    ...
    def _register_extra_handlers(self) -> None:
        ...
        self.app.add_handler(CommandHandler("find_niche", self.cmd_find_niche))
