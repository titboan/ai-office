    ...
    async def handle_task(self, task: str, from_agent: str = "user") -> str:
        ...
        if chat_id and _lead_match:
            ...
            return (
                ...
                f"*Применяется в /supply и /order.*"
            )
...
OZON FBO: поставка идёт на распределительный склад Ozon, он сам развозит по кластерам.
Покажи текущий сток по кластерам (clusters) и суммарно to_order.
...
- total_days_left — суммарный запас (все склады) в днях продаж
- urgency — КРИТИЧНО / СРОЧНО / НОРМА (уже рассчитано)

ФОРМАТ ОТВЕТА — мобильный, без широких таблиц:

═══ WB ═══
🔴 КРИТИЧНО (товар кончится до прихода поставки)
**КБ50** — 30 шт/день | запас: 0 дн
  📦 Склад Иваново: 0 шт → нужно 900 шт
  Итого: **заказать 900 шт**

🟡 СРОЧНО (пора заказывать)
**КБ100** — 5 шт/день | запас: 10 дн
  🚚 В пути: 200 шт
  📦 Склад Иваново: 50 шт → нужно 175 шт
  Итого: нужно 175 шт | в пути 200 шт | **заказать 0 шт**

═══ OZON ═══
🔴 КРИТИЧНО
**КБ100** — 3 шт/день | запас: 3 дн
  Кластер Москва и МО: 5 шт (1,7 дн)
  Кластер Южный: 3 шт (1 дн)
  Итого: **заказать 130 шт**

🟡 СРОЧНО
...

🟢 НОРМА — показывай кратко, без кластеров

В конце: итоговая таблица — товар | площадка | urgency | заказать шт"""
