        ...
        if not prompt and chat_id and any(kw in task.lower() for kw in _supply_kw):
            try:
                ...
                prompt = (
                    ...
                    f"Если ozon_warehouses ПУСТ — FBO данных нет (товар FBS или ещё не поставлен на Ozon FBO).\n"
                    f"В этом случае рекомендуй склады Ozon на основе WB-географии продаж (явно пометив 'оценка по WB'):\n"
                    f"  МО/Москва → ХОРУГВИНО_РФЦ или ТВЕРЬ_РФЦ (~50% объёма)\n"
                    f"  Краснодарский → КРАСНОДАР_РФЦ (~10-15%)\n"
                    f"  СПб/ЛО → САНКТ_ПЕТЕРБУРГ_РФЦ (~7-10%)\n"
                    f"  Урал/Сибирь → ЕКАТЕРИНБУРГ_РФЦ_НОВЫЙ или НОВОСИБИРСК_РФЦ (~7-10%)\n"
                    f"НЕ пиши 'данных нет' без конкретных рекомендаций куда слать товар.\n"
...
• Если ozon_warehouses ПУСТ → товар ещё не поставлен на Ozon FBO (FBS или у поставщика).
  В этом случае дай рекомендации по WB-географии (явно пометив "оценка по WB-данным"):
    МО/Москва → ХОРУГВИНО_РФЦ или ТВЕРЬ_РФЦ (~50% объёма)
    Краснодарский → КРАСНОДАР_РФЦ (~10-15%)
    СПб/ЛО → САНКТ_ПЕТЕРБУРГ_РФЦ (~7-10%)
    Урал/Сибирь → ЕКАТЕРИНБУРГ_РФЦ_НОВЫЙ или НОВОСИБИРСК_РФЦ (~7-10%)
  НЕ пиши просто "данных нет" — всегда давай конкретные рекомендации.

Формат для каждого Ozon-товара КРИТИЧНО/СРОЧНО:

**[Название]**
`[X] шт/день суммарно` · запас [N] дн
*Склады Ozon:* *(из ozon_warehouses если есть, иначе оценка по WB-данным)*
  🏭 [warehouse_name]: [N] шт или [рекомендуемая доля]
