...
Используй формат PETER_AUDIT_PROMPT, но сокращённо."""
    ...
    async def run_daily_digest(self, chat_id: int) -> None:
        ...
        # Авто-триггер Peter→Elina: CTR < 1% → ставим задачу Элине только если есть карточка в БД
        ...
        for m in low_ctr_items[:3]:
            ...
            try:
                async with _pool.acquire() as _conn:
                    _card = await _conn.fetchrow(
                        "SELECT 1 FROM product_cards WHERE chat_id=$1 AND product_id=$2 LIMIT 1",
                        chat_id, article,
                    )
                if not _card:
                    logger.info(f"[Питер/daily_digest] {article} — нет карточки в БД, пропускаем Элину")
                    continue
                await enqueue_task(
                    assigned_agent="elina",
                    payload=f"напиши seo для товара {article}",
