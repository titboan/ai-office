        ...
        if not prompt and chat_id and any(kw in task.lower() for kw in _supply_kw):
            try:
                ...
                prompt = (
                    ...
                    f"ozon_warehouses = [{{'warehouse_name': ..., 'stock': ...}}] — РЕАЛЬНЫЕ склады Ozon из БД.\n"
                    f"ДЛЯ OZON: всегда используй ozon_warehouses — конкретные склады, где хранится товар.\n"
                    f"НИКОГДА не используй WB-данные как прокси для Ozon и не пиши 'данных нет' если ozon_warehouses не пуст.\n"
...
- ozon_warehouses — реальные склады Ozon из БД: [{warehouse_name, stock}]
  ИСПОЛЬЗУЙ ozon_warehouses ДЛЯ РЕКОМЕНДАЦИЙ ПО OZON — это точные данные о том,
  на каких складах хранится товар сейчас. Не используй WB-данные как прокси для Ozon.
...
ПРАВИЛО: всегда используй ozon_warehouses (реальные склады из БД) — никогда не ссылайся
на WB-данные и не пиши "данных нет" если ozon_warehouses не пуст.
